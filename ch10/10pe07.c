...
/*
 * Add a function called nextdate to the program designed in Exercise 10.4 to
 * perform the following task:
 *
 * -Accepts two arguments, one of the structure data containing the present date
 *  and the second an integer that represents the number of days to be added to
 *  the present date.
 * -Adds the days to the present date and returns the structure containing the
 * next date correctly.
 *
 ...
 *	-Wparentheses -Wformat-zero-length 10pe07.c -o binary/10pe07
*/
...
int main(void)
{
	...
	printf("\nSupported year range: " XSTR(YEAR_MIN) " - " XSTR(
	    YEAR_MAX) "\n");
	requestTakeDateInputDDMMYYYY(&d);
	validateDateInput(&d);
	printf("Entered date is:\t");
	displayDateDDMonthYYYYFormatPlain(&d);
	printf("\n");

	requestDaysToAddValidateInput(&daysToAdd);
	addDaysAndDisplayMessage(&d, &daysToAdd);
